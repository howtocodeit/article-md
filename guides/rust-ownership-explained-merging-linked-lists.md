...
git_tag_base: merging-linked-lists
...
og_image_url: https://res.cloudinary.com/dkh7xdo6x/image/upload/v1718047567/linked_lists_og_9483714ac4.jpg
published_at: 2024-06-10
---

# Rust ownership by example: merging linked lists
...
Aria Beingessner's [_Learning Rust With Entirely Too Many Linked Lists_](https://rust-unofficial.github.io/too-many-lists/) is one of my favourite Rust resources. If you haven't worked through it, please do – you'll leave with a fine appreciation for why the borrow checker seems to get in our way, and even less confidence in working with `unsafe` code. This is a good thing, since overconfidence is the enemy of `unsafe`.
...
pub fn merge(
    ...
    let pre_head = Some(Box::new(ListNode { val: 0, next: None }));  :coderef[1]
    let mut tail = pre_head;  :coderef[2]

    while list1.is_some() && list2.is_some() {  :coderef[3]
        if list1.unwrap().val < list2.unwrap().val { :coderef[4]
            tail.unwrap().next = list1;
            list1 = list1.unwrap().next;  :coderef[5]
        } else {
    ...
    tail.unwrap().next = if list1.is_some() { list1 } else { list2 };  :coderef[6]

    pre_head.unwrap().next   :coderef[7]
}
```

`pre_head` is declared as a dummy node that points to the real head of the output list :codelink[1]. In Go, this trick allows us to avoid checking that the output list is non-`nil` before attempting to append new nodes – we know for sure that `pre_head` is non-`nil`, or `Some` in our case. At the end of the function, we return `pre_head`'s `next` node :codelink[7] to get the actual head.
...
We want to keep track of the tail of the output so we can append each node directly to the end of the list. Starting from the head and traversing the whole list for each node we appended would result in quadratic runtime. So I guess we just initialize `tail` as `pre_head` :codelink[2]? Sure, seems legit. 🫠

While there are still nodes in both `list1` and `list2`, we execute the main body of the algorithm :codelink[3]. If only one list has nodes left in it, we know that all of these nodes have values greater than any of the nodes we've already appended to the output. We just tack the remaining list onto the output tail :codelink[6]!
...
The moment we assign `pre_head` to `tail` :codelink[2], it's over. We've given away ownership of `pre_head`, and have nothing to return at :codelink[7].

When we unwrap the heads of `list1` and `list2` to compare them :codelink[4], we doom our loop to failure too:

```rust
pub fn unwrap(self) -> T
```

`Option::unwrap` takes `self` by value. That means we take ownership of both input list heads, but only attempt to put one back :codelink[5].
...
pub fn merge_two_lists_loop_2(
    ...
    let mut head = None;
    let mut next_tail = &mut head;  :coderef[8]

    while list1.is_some() && list2.is_some() {  :coderef[9]
        let head1 = &mut list1;
        let head2 = &mut list2;  :coderef[10]

        let smallest_head = if head1.as_ref().unwrap().val < head2.as_ref().unwrap().val {  :coderef[11]
            head1
        } else {
            head2
        };

        std::mem::swap(smallest_head, next_tail); :coderef[12]

        let new_next_tail = &mut next_tail.as_mut().unwrap().next;
		std::mem::swap(smallest_head, new_next_tail); :coderef[13]
		next_tail = new_next_tail  :coderef[14]
    }

    *next_tail = if list1.is_some() { list1 } else { list2 };  :coderef[15]
...
At :codelink[8], we initialize `head` as `None`. This isn't equivalent to `pre_head`. This is the spot in memory where the true head of our output list will live.
...
The loop condition is the same – `Option::is_some()` takes `&self`, so there's no move here :codelink[9]. But this time we start each iteration by taking mutable references to the head of each list :codelink[10]. We don't have ownership of the input heads, we just point to them.

We'd like to avoid owning a node, so to perform the comparison between values :codelink[11], we change `head1` and `head2` from `&Option<Box<ListNode>>` into `Option<&Box<ListNode>>` using `Option::as_ref`.
...
The _reference_ to the lesser of the two nodes is assigned to `smallest_head`, which represents the node that we want to append to our output list. At this point, `smallest_head` is either a mutable reference to the head of `list1`, or a mutable reference to the head of `list2`.

Now the fun part.

At :codelink[12], we swap the node that `smallest_head` references with the node that `next_tail` references. In other words, we put the node `smallest_head` points to into the slot indicated by `next_tail`.
...
Not ideal. Let's fix it with another swap :codelink[13]. The first argument to `swap` is `smallest_head` again, which is a reference to either `list1` or `list2`. Recall that this now points to `None`. The second argument is a mutable reference to the unsorted _tail_ of the list that we just swapped into `next_tail`: `&mut next_tail.as_mut().unwrap().next`. This is a reference to `next_tail`'s tail, which we'll call `new_next_tail`.
...
Now we've got something swappable :codelink[13]. `smallest_head` was pointing to `None` (the previous value referenced by `next_tail`), so swapping causes `new_next_tail` to once again point to `None`.
...
Thanks to our swaps, we know that `new_next_tail` is `None`, and that this is the exact spot in memory where the next node should be placed. We can just assign `new_next_tail` to `next_tail` at the bottom of the loop :codelink[14].
...
Once one of the lists is exhausted, we just whack the remaining list onto the end of the output :codelink[15]. It's safe to assign directly to `*next_tail` here, because we advanced `next_tail` at the end of the final loop iteration.
...
pub fn merge(
    ...
    match (list1, list2) {  :coderef[16]
        (None, None) => None,  :coderef[17]
        (Some(node), None) | (None, Some(node)) => Some(node),  :coderef[18]
        (Some(mut node1), Some(mut node2)) => {  :coderef[19]
            if node1.val < node2.val {
                node1.next = merge(node1.next, Some(node2)); :coderef[20]
                Some(node1)
            } else {
...
At each recursion, we consider the heads of the two inputs lists as a tuple of nodes :codelink[16].

If both are `None`, then both lists are empty – we return `None` to take the final spot in the output list :codelink[17].

In practice, this only happens if both input lists are empty to begin with. Each iteration removes at most one node from one of the input lists. The second match arm catches the case where one list is empty and one isn't :codelink[18]. In this case, we just return the matched node, and it will bring the remainder of its list with it.

The interesting case is when both heads are `Some` :codelink[19]. We identify the node with the lowest value, and set its `next` field to the result of a recursive `merge` call, passing `node.next` as one input, and the entirety of the list with the larger head as the second.

Note that matching the heads of each list takes ownership. We give that ownership away when we call `merge` recursively :codelink[20], but the immediate reassignment to `nodeN.next` means that the complete node is ours to pass back up the call chain once the recursion returns.

This more readable code comes at the cost of the stack space required to hold the recursive calls (i.e. it has _O(n)_ space complexity).
...
pub fn merge_two_lists(
    ...
    loop {
        match (list1, list2) {
            (Some(mut head1), Some(mut head2)) => { :coderef[21]
                let smallest_head = if head1.val < head2.val {
                    list1 = head1.next.take(); :coderef[22]
                    list2 = Some(head2); :coderef[23]
                    head1
                } else {
                ...
                next_tail = &mut next_tail.insert(smallest_head).next; :coderef[24]
            }
            (tail, None) | (None, tail) => { :coderef[25]
                *next_tail = tail;
                return head
            }
        }
...
Ownership of the heads of both lists is acquired at the top of each iteration :codelink[21], which requires us to put something back before the start of the next.

Let's assume `list1`'s head has the smaller value. We assign `head1`'s `next` node directly to `list1` :codelink[22], setting up `list1` for the next iteration and keeping `head1` for ourselves. `Option::take` gives us ownership of everything after `head1` and leaves `None` in its place. I find this more intuitive than `mem::swap`.

:::aside{type=info hideImage=true}
`Option::take` is in fact implemented in terms of `mem::replace`:

```rust crate=rust path=src/core/option.rs
impl<T> Option<T>
    #[inline]
...
Having no further use for the larger `head2`, we hand ownership straight back to `list2` :codelink[23].

Rather than do the `swap` dance with `next_tail`, we invoke `Option::insert` to put `smallest_head` in its place. We know that `next_tail` is `None` at the top of the loop, so there's no risk of overwriting a node. Finally, we assign a mutable reference to the new tail's `next` node to `next_tail`, setting up the next iteration :codelink[24].

In the case where either of the two lists is `None` :codelink[25], our work is done. Just slot the
...
::discussion{title="Ownership by example: merging linked lists in Rust"}
